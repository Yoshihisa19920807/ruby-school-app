class CoursesController < ApplicationController
  before_action :set_course,
                only: %i[show edit update destroy approve unapprove analytics]
  ...
  def teaching
    ...
    @ransack_courses =
      Course.where(user: current_user).ransack(
  ...
  def purchased
    ...
    @q =
      Course
        ...
        .ransack(params[:search_courses], search_key: :search_courses)
  ...
  def review_pending
    ...
    @q =
      Course
        .includes(:enrollments)
        .where(enrollments: { user: current_user, rating: nil })
        .ransack(params[:search_courses], search_key: :search_courses)
    @pagy, @courses = pagy(@q.result.includes(:enrollments, :tags))
    @tags = Tag.all
    render 'index'
  ...
  def create
    ...
    respond_to do |format|
      if @course.save
        ...
        format.html do
          redirect_to course_course_wizards_path(@course),
  ...
  def destroy
    ...
    if @course.destroy
      respond_to do |format|
        format.html do
          redirect_to teaching_courses_path,
