class Course < ApplicationRecord
  ...
  scope :purchased,
        ->(user) { includes(:enrollments).where(enrollments: { user: user }) }
  ...
  def self.levels
    %w[Beginner Intermidiate Advanced]
  end
  ...
  def update_average_rate
    ...
    if enrollments.any? && enrollments.where.not(rating: nil).any?
      update_column :average_rating,
                    (enrollments.average(:rating).round(2).to_f)
    else
